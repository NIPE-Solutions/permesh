...
impl Code {
    pub(crate) fn row(self, instance: &str) -> Row {
        let (stage, message, next) = match self {
            ...
            Self::CredentialUnavailable => (Stage::Credentials, "A configured credential reference could not be resolved.", format!("Review the configured credential source and its read permissions. Set its explicit environment or remote bootstrap reference; use permesh auth login {instance} --credential SLOT only for a direct keychain slot.")),
