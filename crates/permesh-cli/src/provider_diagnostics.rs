...
impl Code {
    pub(crate) fn row(self, instance: &str) -> Row {
        let (stage, message, next) = match self {
            ...
            Self::CredentialUnavailable => (Stage::Credentials, "A configured credential source could not be resolved.", format!("Inspect the credential source for {instance}. Set its environment reference, use auth login for a keychain slot, check the remote resolver bootstrap, or refresh the selected temporary AWS profile. Then rerun doctor --details.")),
