...
fn fixture(name: &str) -> Value {
    let source = match name {
        "user" => include_str!("fixtures/schema1/user.json"),
        "admins" => include_str!("fixtures/schema1/admins.json"),
        "orphaned" => include_str!("fixtures/schema1/orphaned.json"),
        "snapshot" => include_str!("fixtures/schema1/snapshot.json"),
        _ => unreachable!(),
    };
    serde_json::from_str(source).unwrap()
}

// These complete static fixtures freeze schema 1 independently of core's serde derives.
// Removing/renaming fields, changing enum tags, dropping nulls or reordering arrays fails.
#[tokio::test]
async fn demo_query_and_snapshot_dtos_preserve_schema_one_fixtures() {
    let snapshot = permesh_provider_demo::DemoProvider::new("demo")
        .discover()
        .await
        .unwrap();
    assert_eq!(
        serde_json::to_value(schema1::Snapshot::from(&snapshot)).unwrap(),
        fixture("snapshot")
    );
    ...
    assert_eq!(
        serde_json::to_value(schema1::UserAccess::from(&user)).unwrap(),
        fixture("user")
    );
    let admins = query_admins(&snapshots, &aliases).unwrap();
    assert_eq!(
        serde_json::to_value(schema1::AdminAccess::from(&admins)).unwrap(),
        fixture("admins")
    );
    let orphaned = query_orphaned(&snapshots, &aliases, &["demo".into()]).unwrap();
    assert_eq!(
        serde_json::to_value(schema1::OrphanedAccess::from(&orphaned)).unwrap(),
        fixture("orphaned")
    );
}
...
#[test]
fn resolution_tags_and_unresolved_grant_nulls_remain_schema_one() {
    use permesh_core::*;
    let account = Account {
    ...
    let expected: Value =
        serde_json::from_str(include_str!("fixtures/schema1/resolutions.json")).unwrap();
    assert_eq!(
        serde_json::to_value(schema1::AdminAccess::from(&result)).unwrap(),
        expected
    );
}

#[test]
fn every_classification_enum_has_an_explicit_schema_one_spelling() {
    use permesh_core::*;
    let kinds = [
        IdentityKind::Human,
    ...
    let certainties = [Certainty::Observed, Certainty::Inferred, Certainty::Unknown];
    ...
    let value = serde_json::to_value(schema1::Snapshot::from(&snapshot)).unwrap();
    ...
    assert_eq!(
        ...
        ["human", "external", "service", "bot", "unknown"]
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        ["active", "inactive", "external", "service", "unknown"]
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        ["observed", "inferred", "unknown", "observed", "inferred"]
    );
    let reasons = [
    ...
    let value = serde_json::to_value(schema1::OrphanedAccess::from(&orphaned)).unwrap();
...
#[test]
fn cli_demo_query_results_match_complete_schema_one_fixtures() {
