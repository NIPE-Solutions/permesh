...
const VISIBILITY: &[&str] = &[
    ...
    "Identity kind and affiliation are unknown. Archived means inactive; otherwise suspended means suspended. Active means both suspended and archived are explicitly false; otherwise missing status evidence remains unknown. Deleted users, groups, memberships, resources and grants are not enumerated.",
    "An existing OAuth access token is required; credentials are not refreshed. Health checks probe at most one user and do not establish complete directory visibility.",
];
...
impl GoogleProvider {
    ...
    fn record(&self, value: &Value) -> Result<(Account, Identity, bool), ProviderError> {
        ...
        Ok((
            ...
            Identity {
                id: format!("google:{}:{id}", self.customer_id),
                kind: IdentityKind::Unknown,
                affiliation: Affiliation::Unknown,
                status,
                verified_emails: vec![email.into()],
