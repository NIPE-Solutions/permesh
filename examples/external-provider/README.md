...
This script is a protocol example, not an installed native provider. Run it
deliberately with Python and validate its records with the Rust protocol crate.
Permesh executes explicitly trusted native providers through the separate
[external-provider workflow](../../docs/external-providers.md); it never scans
and runs repository scripts automatically.
...
developer-tool output, separate from CLI access schema 2 and control schema 1.
...
it provides no live-process deadlines. These peer/validator tests do not qualify host trust registration or process-tree
supervision; the native host has its own tests. Native execution is not a sandbox.
