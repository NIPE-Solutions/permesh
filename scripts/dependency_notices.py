...
# Copyright (c) 2026 Permesh contributors
# Adapted from permesh-providers/scripts/provider_notices.py for the CLI graph.
# Original: MIT OR Apache-2.0; used here under its MIT option (see ../LICENSE).
...
def reachable(metadata):
    ...
    roots = [p['id'] for p in metadata['packages'] if p['name'] == 'permesh-cli']
    if len(roots) != 1 or metadata.get('resolve') is None:
        raise ValueError('CLI dependency graph is missing or ambiguous')
    ...
    while pending:
        ...
        if len(seen) >= 4096 or package_id not in packages or package_id not in nodes:
            raise ValueError('CLI dependency graph is invalid or oversized')
        seen.add(package_id)
...
def bundle(root, metadata):
    ...
    sections = ['Permesh CLI distribution licenses and notices', '', project.rstrip(), '',
    ...
    if len(result) > MAX_NOTICE_BYTES:
        raise ValueError('combined license exceeds the CLI package limit')
    return result
