...
For `auth_status` only, remote-backed or explicit temporary AWS-profile instances
may have state `configured`. This means their declaration was inspected;
credential availability and provider authentication remain unverified. No remote
store, bootstrap credential or AWS profile file was accessed. `complete: true` and exit 0 indicate this inspection completed with
...
`external_review` may also include `aws_profile` with `schema_version: 1`, backend
`aws_temporary_profile`, `credentials_file`, `profile` and the three delivered
credential-slot names. Review does not read the profile file or resolve its values.
Legacy configurations omit these fields and retain their existing output.
