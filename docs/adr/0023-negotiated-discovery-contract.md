...
Status: Accepted for prerelease implementation; wire 5 is not declared stable.
...
without dropping information. Reinterpreting those fields would break deployed
native providers and make historical evidence ambiguous.

## Decision

Keep every legacy draft unchanged. Add an explicit `negotiated_v5` discovery
selection in workspace configuration. Wire 5 introduces independent record DTOs
...
Wire 5 is selected because 1–4 already identify incompatible historical exchanges.
It is a new incompatible record contract, not a number assigned to one new
operation. A future compatible operation does not need a new wire version.
There is currently one supported version in this negotiated family. The host pins
it explicitly; no highest-version search, automatic probing, retry against a
legacy draft, or silent downgrade occurs.
...
A provider must implement wire 5 before its workspace opts in.
...
See [wire 5](../provider-protocol/negotiated-v5.md) and
