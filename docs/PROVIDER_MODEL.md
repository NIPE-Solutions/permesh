...
External provider protocol is specified in `provider-development.md`; executable loading is deferred. No empty official AWS/Cloudflare crates are created. Google supplies only directory accounts and identities; it does not invent access grants. See [Google scope](providers/google.md).
