...
- [directories 6.0.0](https://github.com/soc/directories-rs) (MIT OR Apache-2.0): platform application-data locations for the CLI trust store. Small platform-convention library; no network implementation. The CLI exposes an explicit absolute `PERMESH_DATA_DIR` override for isolated local environments.
