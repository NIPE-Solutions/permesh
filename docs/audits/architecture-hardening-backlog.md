...
| 2. Boundary freeze | P0 / 1 | Historical wire transcripts; wire-owned records/capabilities and private validated mapping; CLI-owned query/snapshot/control DTOs; official emitter projection; versioning ADR. | Old transcripts and full JSON fixtures survive extraction; malformed input still fails closed; no implicit schema change. | Implemented; follow-up CI pending |
...
| 7. Official onboarding | P1 / 6 | Add/install/verify/trust/setup/approve orchestration; concise consent; resumable prompts and CI flags. | Add/auth/query requires no manual hashes; rejection executes nothing; updates never silently adopt pins; expert workflow preserved. | Implemented for GitHub; follow-up CI pending |
...
test remains opt-in); cross-platform follow-up CI remains a merge gate.
...
The next P0 slice is the independent identity/resource/access-evidence model and
its output-schema migration. Frozen DTOs are preparation for that change, not a
claim that the present domain or operation-specific drafts are stable.
