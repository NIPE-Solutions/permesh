...
JSON reports can expose account names, infrastructure names, access relationships, and immutable IDs. Protect redirected files with appropriate directory permissions/ACLs. Explicit snapshot, offboarding plan and HTML exports contain sensitive review metadata. Writes use bounded validation, private temporary files and atomic publication. Existing destinations require explicit snapshot `--overwrite`; offboarding outputs always require new paths. Windows uses inherited ACLs. Offline snapshot inspection and comparison require no credentials or provider execution. Permesh uploads no reports.
